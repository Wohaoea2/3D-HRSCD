...
def train(train_loader, net, criterion, optimizer, scheduler, val_loader):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            # # 计算 ConLoss
            con_loss = ConLoss(margin=2.0).cuda()
            seg_diff = torch.abs(outputs_A - outputs_B)
            seg_diff = F.interpolate(seg_diff.float(), size=(512 // 4, 512 // 4), mode='nearest')
            x1 = out_change
            out_change = torch.sigmoid(out_change)

            bcd_mask = F.interpolate(out_change.float(), size=(512 // 4, 512 // 4), mode='nearest')
            bcd_true_mask = F.interpolate(labels_bn.float(), size=(512 // 4, 512 // 4), mode='nearest')
            bcd_mask = bcd_mask.cuda()
            bcd_true_mask = bcd_true_mask.cuda()

            out_change = x1

            loss_con = con_loss(seg_diff, bcd_mask, bcd_true_mask) * 0.1

            loss = loss_seg + loss_bn + loss_sc + loss_con
            ...
            train_con_loss.update(loss_con.cpu().detach().numpy())
