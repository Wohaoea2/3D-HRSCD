...
from models.HRSCD_3D import HRNet_3D as Net
...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--pred_dir', required=False, default='/home/results', help='directory to output masks')
        parser.add_argument('--chkpt_path', required=False, default='/home/3D-HRSCD/trained_model/HRNet_3D_46e_mIoU73.79_Sek23.64_Fscd62.68_OA87.85.pth', help='path to model checkpoint')
...
def main():
    ...
    net.eval()
    opt.test_dir = '/home/data/SECOND2/val'
    test_set = SP.Data_test(opt.test_dir)
...
def predict(net, pred_set, pred_loader, pred_dir, flip=False, index_map=False, intermediate=True):
    ...
    for vi, data in enumerate(pred_loader):
        ...
        if intermediate:
            ...
            io.imsave(pred_A_path, SP.Index2Color(pred_A.numpy()))
            io.imsave(pred_B_path, SP.Index2Color(pred_B.numpy()))
        ...
        io.imsave(pred_A_path, SP.Index2Color(pred_A))
        io.imsave(pred_B_path, SP.Index2Color(pred_B))   
        print(pred_A_path)
        if index_map:
