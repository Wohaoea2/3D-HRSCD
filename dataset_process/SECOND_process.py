...
# num_classes = 5
# ST_COLORMAP = [[255,255,255], [0,0,255], [128,128,128],  [130,87,87], [255,0,0]]
# ST_CLASSES = ['unchanged', 'water', 'Farmland', 'desert',  'building']
...
# root = '/home/dml307/data/public/Landsat/'
...
def read_RSimages(mode, rescale=False):
    ...
    # change_label_dir = os.path.join(root, mode, 'label')  # 新增变化标签文件夹路径
    # To use rgb labels:
    #label_A_dir = os.path.join(root, mode, 'label1_rgb')
    #label_B_dir = os.path.join(root, mode, 'label2_rgb')
    
    data_list = os.listdir(img_A_dir)
    # imgs_list_A, imgs_list_B, labels_A, labels_B, change_labels = [], [], [], [], []  # 新增变化标签列表
    ...
    for it in data_list:
        ...
        if (it[-4:]=='.png'):
            ...
            # change_label_path = os.path.join(change_label_dir, it)  # 新增变化标签路径
            ...
            # change_labels.append(io.imread(change_label_path))  # 新增变化标签读取
        count+=1
    ...
    # return imgs_list_A, imgs_list_B, labels_A, labels_B, change_labels
...
class Data(data.Dataset):
    def __init__(self, mode, random_flip = False):
        ...
        # self.imgs_list_A, self.imgs_list_B, self.labels_A, self.labels_B, self.change_labels = read_RSimages(mode)  # 新增变化标签 
    ...
    def __getitem__(self, idx):
        ...
        # change_label = self.change_labels[idx]  # 新增变化标签
        # if self.random_flip:
        #     img_A, img_B, label_A, label_B, change_label = transform.rand_rot90_flip_MCD(img_A, img_B, label_A, label_B, change_label)  # 新增变化标签处理 
        # return F.to_tensor(img_A), F.to_tensor(img_B), torch.from_numpy(label_A), torch.from_numpy(label_B), torch.from_numpy(change_label)  # 返回变化标签

        if self.random_flip:
            img_A, img_B, label_A, label_B= transform.rand_rot90_flip_MCD(img_A, img_B, label_A, label_B)  # 新增变化标签处理 
