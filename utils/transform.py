...
# def rand_flip_MCD(img1, img2, label1, label2, change_label):
#     r = random.random()
#     # showIMG(img.transpose((1, 2, 0)))
#     if r < 0.25:
#         return img1, img2, label1, label2, change_label
#     elif r < 0.5:
#         return np.flip(img1, axis=0).copy(), np.flip(img2, axis=0).copy(), np.flip(label1, axis=0).copy(), np.flip(label2, axis=0).copy(), np.flip(change_label, axis=0).copy()
#     elif r < 0.75:
#         return np.flip(img1, axis=1).copy(), np.flip(img2, axis=1).copy(), np.flip(label1, axis=1).copy(), np.flip(label2, axis=1).copy(), np.flip(change_label, axis=1).copy()
#     else:
#         return img1[::-1, ::-1, :].copy(), img2[::-1, ::-1, :].copy(), label1[::-1, ::-1].copy(), label2[::-1, ::-1].copy(), change_label[::-1, ::-1].copy()
